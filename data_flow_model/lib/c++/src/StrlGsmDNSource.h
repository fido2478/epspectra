...
class StrlGsmDNSource : public StrlSource<oType> {
protected:
  FILE* fp;
  int repeat;
#ifdef _PERF
...
public:
  virtual const char *name() { return "StrlGsmDNSource"; }
  virtual int work(VrSampleRange output, void *o[]);
  StrlGsmDNSource(char* file, int repeat = 0);
  virtual ~StrlGsmDNSource();
};

template<class oType> int
StrlGsmDNSource<oType>::work(VrSampleRange output, void *ao[])
{
  oType **o= (oType **)ao;
  int i;
  int return_size;
...
StrlGsmDNSource<oType>::initialize() {
#ifdef _PERF
...
template<class oType>
StrlGsmDNSource<oType>::StrlGsmDNSource(char* filename, int arg_repeat = 0)
{
  repeat = arg_repeat;
...
template<class oType>
StrlGsmDNSource<oType>::~StrlGsmDNSource()
{
  fclose(fp);
}
#endif
