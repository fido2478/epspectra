...
protected: //the guppi stuff needs this (FIX)
  ...
  int is_synced(VrSampleIndex index) {
    if(WP >= index)
       return 1;
    //return 0; //marked in order to skip data in Esterel
  }
...
public:
  ...
  int StrlFitRange(VrSampleIndex pendingWP, int requestSize,VrSampleRange testrange); 
  VrSampleRange* StrlFitRangeForMainSink(VrSampleRange org_input);
  int StrlComputeForMainSink();
  void init_base();
