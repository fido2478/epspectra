...
#endif 
//#ifdef PERFMON
//extern static int mutual_section=0;
//#endif

unsigned int StrlSigProc::getBufferSize()
{
    return (*outBuffer)->buffersize();
}
...
/******************************************************
 * This function is for data_flow model
 ******************************************************/

int StrlSigProc::StrlFitRange(VrSampleIndex pendingWP, int requestSize,VrSampleRange inputrange )
{
...
VrSampleRange* StrlSigProc::StrlFitRangeForMainSink(VrSampleRange org_input )
  //VrSampleIndex pendingWP, int requestSize,VrSampleRange inputrange )
{
  ...
  requestSize=((requestSize/outputSize)*outputSize);
  output.index=pendingWP;
  output.size=requestSize;

  VrSampleRange inputrange[numberInputs];
  forecast(output,inputrange);
  ...
  str->index=inputrange[0].index;
  str->size=inputrange[0].size;
  return str;
}
...
unsigned long StrlSigProc::Data_Flow_Work(int return_type)
{ 
...
int StrlSigProc::StrlComputeForMainSink()
{
  VrSampleRange& r=myMarkedData;
  if(!myWriterLL) return 0; //no data marked
  VrSampleRange *inputs=(VrSampleRange *) THREAD_GETSPECIFIC(inputs_forecasted);
  void *inputptrs[numberInputs];
  void *outputptrs[numberOutputs];
  int not_complete=0; //flag
  unsigned long complete=0; //units finished
  ...
  if(numberInputs > 0 ) {
    for(unsigned int i=0;i<numberInputs;i++) {
      if(getUpstreamModuleN(i)->compute()<0)
	not_complete--;
    }
    if(not_complete<0)
 ...
 unmark_and_return:
      if(not_complete<0) {
	printf("%s entered unmark_and_return section: not_complete %d\n",name(),not_complete);
	markedWP = r.index+complete;
#ifdef PERFMON
  ...
  r.size = 0;
#ifdef DEBUG
  printf("%s (compute-return [%d]): complete %d\n\n",name(),not_complete,complete);
#endif
  return not_complete;
}
...
int StrlSigProc::downcast(VrSampleRange input, VrSampleRange* output) {
  /* 1:1 ratio with no history */
  output->index=input.index; /* ! do not subtract history ! */
  output->size=input.size; /* + history */
#ifdef DEBUG
