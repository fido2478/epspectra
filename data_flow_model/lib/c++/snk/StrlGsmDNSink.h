...
#ifndef _STRLGSMDNSINK_H_
#define _STRLGSMDNSINK_H_
#define __volatile__
...
class StrlGsmDNSink : public StrlSink<iType> {
private:
  int sockfd;
  int outputsize;
  int sleep_time;
...
public:
  virtual const char *name() { return "StrlGsmDNSink"; }
...
//	cout<<"In the "<<name()<<"initializing"<<endl;
  ...
  StrlGsmDNSink(char* hostname, int port) {
	next_time = 0;
  ...
  virtual ~StrlGsmDNSink() { }
};

template<class iType> int
StrlGsmDNSink<iType>::work(VrSampleRange output, 
