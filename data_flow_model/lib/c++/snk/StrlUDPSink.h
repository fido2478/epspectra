			...
			VrSampleRange inputs[], void *ai[])
{
  ...
  if (this_time < next_time){
    ...
    //printf("sleeping* : %lu usec \n", tmp);//average=55000(>45), 55000 max=110000
    if(tmp>170000){
      ...
      tmp=tmp*2.090901;//2.2115385(52);//2.3(50);//2.5555556(45);//2.0909091;//1.4375;
      usleep((int)(tmp));
      printf("sleeping115 : %lu usec \n", tmp);}
    else if (tmp>100000){//(105000/60000)
      tmp=tmp*1.9090909;//2.0192308(52)//2.1(50);//2.3333333(45);//1.9090909;//1.75;//1.3125;
      usleep((int)(tmp));
      printf("sleeping105 : %lu usec \n", tmp);}
    else if (tmp>90000){//95000/75000
      tmp=tmp*1.7272727;//1.8269231(52)//1.9(50);//2.1111111;//1.7272727;//1.2666667;//1.1875;//2
      usleep((int)(tmp));
      printf("sleeping95 : %lu usec \n", tmp);}
    else if (tmp>80000){//85000/80000
      tmp=tmp*1.8888889;//1.5454545;//1.0625;
      usleep((int)(tmp));
